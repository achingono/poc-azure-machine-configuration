Configuration ServerConfiguration {
    ...
    # https://github.com/dsccommunity/WebAdministrationDsc/wiki
    ...
    Node localhost {
        ...
        Script WebsitePath {
            ...
            TestScript = {
                Test-Path "C:\inetpub\$using:siteName"
            }
            SetScript  = {
                New-Item -ItemType Directory -Path "C:\inetpub\$using:siteName" -Force;
            }
        }

        # Create IISSite
        Website $siteName {
            DependsOn = "[Script]WebsitePath", '[WindowsFeature]IIS', "[WebAppPool]$applicationPool"
            Ensure          = "Present"
            Name            = $siteName
            State           = "Started"
        ...
        Script DownloadWebPackage {
            ...
            TestScript = {
                Test-Path "$using:downloadPath\$using:packageName"
            }
            SetScript  = {
                New-Item -ItemType Directory -Path $using:downloadPath -Force;
        ...
        Script DeployWebPackage {
            DependsOn = "[Script]DownloadWebPackage", "[MsiPackage]WebDeploy"
            GetScript  = {
                @{
                    Result = ""
                }
